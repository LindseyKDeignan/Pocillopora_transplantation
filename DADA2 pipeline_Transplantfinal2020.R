...
##########################################
#To create the nMDS add metadata to the asv table
MDS <- read.csv("Transplant_asv_final_rare_MDS.csv", header = T)

#multivariate analyses require separate matrices for data & covariates
Dat <- MDS[,4:8470]
Fac1 <- MDS[,1:3]
...
DatL.bc = vegdist(DatL.sqrt, method = "bray") 

DatL.mds = metaMDS(DatL.bc, autotransform = F, trace = F, trymax=50)
DatL.mds 
...
#Spider plot of nMDS
scrs <- scores(DatL.mds, display = 'sites')
scrs <- cbind(as.data.frame(scrs), Reef = Fac1$Reef)
cent <- aggregate(cbind(NMDS1, NMDS2) ~ Reef, data = scrs, FUN = mean)
segs <- merge(scrs, setNames(cent, c('Reef','oNMDS1','oNMDS2')),
              by = 'Reef', sort = FALSE)

Fig1 <- ggplot(scrs, aes(x = NMDS1, y = NMDS2, colour = Reef)) +
